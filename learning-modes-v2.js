(() => {
  ...
  function startExam(){
    ...
    function renderQuestion(){
      ...
      examShell('📝 Exam Mode — الكسور', 'ما في Hints ولا تصحيح الآن. جاوب بهدوء، والنتيجة الكاملة بتطلع بعد التسليم.', `
        <section class="panel exam-status"><div class="topline"><b>السؤال ${index+1} من ${EXAM.length}</b><span>${answers.filter(v=>v!==null).length}/${EXAM.length} مجاب</span></div><div class="progress"><span style="width:${pct}%"></span></div></section>
        <section class="card exam-question-card"><div class="quiz-head"><span>${esc(q.source)}</span><span>${esc(q.concept)}</span></div><div class="question">${math(q.q)}</div><div class="answers" id="examAnswers">${q.a.map((a,n)=>`<button class="answer ${selected===n?'selected':''}" data-n="${n}">${math(a)}</button>`).join('')}</div><div class="actions exam-actions"><button class="btn btn-soft" id="examPrev" ${index===0?'disabled':''}>السابق</button>${index<EXAM.length-1?'<button class="btn btn-primary" id="examNext">التالي</button>':'<button class="btn btn-primary" id="examSubmit">تسليم الامتحان</button>'}<button class="btn btn-soft" id="examQuit">خروج</button></div><div id="examMsg"></div></section>`);
      document.querySelectorAll('#examAnswers .answer').forEach(b => b.onclick = () => { answers[index]=Number(b.dataset.n); document.querySelectorAll('#examAnswers .answer').forEach(x=>x.classList.remove('selected')); b.classList.add('selected'); });
      ...
      document.getElementById('examSubmit')?.addEventListener('click',()=>{
        ...
        if(missing){ document.getElementById('examMsg').innerHTML=`<div class="error">باقي ${missing} سؤال بدون إجابة. فيك ترجع تجاوبهم، أو تسلّم كما هو.<div class="actions"><button class="btn btn-warning" id="submitAnyway">سلّم كما هو</button></div></div>`; document.getElementById('submitAnyway').onclick=finishExam; }
        else finishExam();
      });
    }
    ...
    async function finishExam(){
      ...
      const details=EXAM.map((q,i)=>({id:q.id,concept:q.concept,question:q.q,selected:answers[i]===null?null:q.a[answers[i]],correct_answer:q.a[q.ok],is_correct:answers[i]===q.ok,explanation:q.why,source:q.source}));
      ...
      examShell('📊 نتيجة الامتحان', `أجبت صح على ${correct} من ${EXAM.length}.`, `<section class="panel exam-result-summary"><div class="exam-score">${correct}/${EXAM.length}</div><div class="stats"><div class="stat">النسبة<b>${score}%</b></div><div class="stat">صحيح<b>${correct}</b></div><div class="stat">خطأ/فارغ<b>${EXAM.length-correct}</b></div><div class="stat">المدة<b>${Math.max(1,Math.round(duration/60))} د</b></div></div><div class="section-title">🎯 يحتاج مراجعة</div><div class="concept-chips">${missed.length?missed.map(c=>`<span>${esc(c)}</span>`).join(''):'<span class="all-good">كل المفاهيم ممتازة 🎉</span>'}</div><div id="examSaveState" class="muted">عم نحفظ نتيجة الامتحان…</div><div class="actions"><button class="btn btn-primary" id="backToStudent">رجوع لصفحتي</button></div></section><section class="panel"><div class="section-title">🔎 مراجعة كل سؤال</div>${rows}</section>`);
      ...
      try{
        ...
        await saveExamDetails({quiz_slug:'fractions-pages-54-57-exam',question_results:details,score,duration_seconds:duration},token).catch(()=>{});
        const el=document.getElementById('examSaveState');
        if(el) el.innerHTML=award.already_awarded?'<div class="success">✅ النتيجة انحفظت. التكرار ما يجمع XP إضافي تلقائيًا.</div>':`<div class="award-pop">🎉 النتيجة انحفظت — +${award.award.xp} XP و 🪙 +${award.award.reward_points} نقطة</div>`;
      }catch{ const el=document.getElementById('examSaveState'); if(el) el.innerHTML='<div class="error">النتيجة ظاهرة، لكن صار خطأ بالحفظ. خبر الأهل قبل إعادة الامتحان.</div>'; }
    }
    renderQuestion();
  }
  ...
  async function addParentExamReport(){
    ...
    const heading=document.querySelector('.hero h1'); if(!heading||!heading.textContent.includes('لوحة الأهل')) return;
    ...
    try{
      ...
      const section=document.createElement('section'); section.id='parentExamReport'; section.className='panel';
      const visible=exams.filter(a=>learnerById[a.learner_id]?.slug!=='test');
      if(!visible.length) section.innerHTML='<div class="section-title">📝 تقارير الامتحانات</div><div class="empty">ما في امتحانات محفوظة بعد.</div>';
      else section.innerHTML=`<div class="section-title">📝 تقارير الامتحانات</div>${visible.map(a=>{const l=learnerById[a.learner_id];const rs=Array.isArray(a.metadata?.question_results)?a.metadata.question_results:[];const total=a.metadata?.total_count??(rs.length||10);const correct=a.metadata?.correct_count??rs.filter(r=>r.is_correct).length;return `<details class="parent-exam"><summary>${esc(l?.display_name||'طالب')} — ${Math.round(a.percentage||0)}% — ${a.submitted_at?new Date(a.submitted_at).toLocaleDateString('ar'):'—'}</summary><div class="review-body"><div>الصحيح: <b>${correct}/${total}</b></div>${rs.map((r,i)=>`<div class="parent-exam-row">${r.is_correct?'✅':'❌'} ${i+1}. ${math(esc(r.question||''))}<br><span class="muted">إجابته: ${r.selected==null?'بدون إجابة':math(esc(r.selected))} · الصحيح: ${math(esc(r.correct_answer||''))}</span></div>`).join('')}</div></details>`;}).join('')}`;
  ...
  function sync(){ fixStudentFooterLink(); addModeCards(); addParentExamReport(); }
