-- Transparently upgrade legacy unsalted SHA-256 learner PIN hashes to bcrypt on successful login.
...
begin
  ...
  for r in
    select *
    from public.learner_access_tokens
  ...
  loop
    if r.token_hash like '$2%' then
      ...
      update public.learner_access_tokens
      set token_hash = extensions.crypt(p_pin, extensions.gen_salt('bf', 10)),
          last_used_at = now()
      where id = r.id;
      return r.id;
    end if;
...
select id,
       'security.learner_pin_hashing',
       '{"current":"bcrypt","cost":10,"legacy":"sha256","upgrade_strategy":"upgrade_on_successful_login","pin_format":"8_digits"}'::jsonb,
       'Learner PIN hashes are upgraded from legacy SHA-256 to bcrypt transparently after a successful login.'
