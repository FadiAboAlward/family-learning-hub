...
declare
  ...
  legacy_sha text;
begin
  if p_pin is null or p_pin !~ '^\d{8}$' then return null; end if;
...
select id,'security.learner_pin_hashing','{"current":"bcrypt","cost":10,"legacy":"sha256","upgrade_strategy":"upgrade_on_successful_login","pin_format":"8_digits"}'::jsonb,
