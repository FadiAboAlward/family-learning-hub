(() => {
  ...
  function genericCopy(){document.querySelectorAll('.role-card .muted').forEach(el=>{if((el.textContent||'').includes('متابعة آية ومحمد'))el.textContent='متابعة المتعلمين والنتائج والمكافآت.';});}
  async function renderDynamicLogin(){
    if(typeof shell!=='function'||typeof api!=='function')return originalStudentLogin?.();
    shell('🧑‍🎓 دخول الطالب','اختَر اسمك، ثم اكتب رمزك.',`<section class="panel"><div class="choice-grid" id="dynamicLearnerGrid"><div class="loading-card">جارِ تحميل الحسابات…</div></div><div id="pinArea"></div><div class="actions"><button class="btn btn-soft" id="backHome">رجوع</button></div></section>`);
    document.getElementById('backHome')?.addEventListener('click',()=>{location.hash='';});
    try{const d=await api('learner_choices'),learners=d.learners||[],grid=document.getElementById('dynamicLearnerGrid');if(!grid)return;grid.innerHTML=learners.length?learners.map(l=>`<button class="profile-card" data-dynamic-learner="${safe(l.slug)}"><span class="big-emoji">${safe(l.avatar_emoji||'🧑‍🎓')}</span><div class="title">${safe(l.display_name)}</div>${l.is_test?'<div class="muted">للتجارب فقط</div>':''}</button>`).join(''):'<div class="empty">لا توجد حسابات طالب مفعلة.</div>';grid.querySelectorAll('[data-dynamic-learner]').forEach(btn=>btn.addEventListener('click',()=>{const slug=btn.getAttribute('data-dynamic-learner')||'',name=btn.querySelector('.title')?.textContent||slug;if(typeof showPin==='function')showPin(slug,name);}));}
