(() => {
  ...
  async function startExam(slug){
    ...
    const answerSaveErrors=new Set();
    ...
    function answerPending(qid){return answerSaveChains.has(qid)}
    ...
    function render(){
      ...
      const row=questions[index],q=row.question,selected=answers.get(row.question_id),isFlagged=flagged.has(row.question_id),pending=answerPending(row.question_id),saveError=answerSaveErrors.has(row.question_id);
      const opts=(q.options||[]).map(o=>{const pos=Number(o.position),sel=pos===selected;return`<button class="answer exam-v3-answer ${sel?'selected':''}" data-pos="${pos}" ${submitting?'disabled':''}><span class="answer-number">${sel?'✓':pos}</span><span>${renderMath(o.content)}</span></button>`}).join('');
      const nav=questions.map((x,i)=>`<button class="${statusClass(x.question_id,i)}" data-i="${i}" ${saving||submitting?'disabled':''}>${flagged.has(x.question_id)?'🚩 ':answers.has(x.question_id)?'✓ ':''}${i+1}</button>`).join('');
      const resume=session.resumed?'<div class="flh-resume-note">↩️ رجعناك لامتحانك، وإجاباتك المحفوظة موجودة.</div>':'';
      const saveStatus=saveError?'⚠️ تعذر حفظ آخر اختيار. اختره مرة ثانية.':pending?'✓ تم اختيار الإجابة — يتم حفظها بالخلفية…':selected?'✓ الإجابة محفوظة تلقائيًا.':'اختر إجابتك.';
      shell(`📝 ${safe(session.quiz.title)}`,`السؤال ${index+1} من ${questions.length} — لا يظهر التصحيح قبل التسليم.`,`<section class="panel flh-touch-quiz flh-exam-swipe-zone">${resume}<div class="exam-status"><div class="topline"><b>السؤال ${index+1} من ${questions.length}</b><span>${answeredCount()}/${questions.length} مجاب</span></div><div class="flh-exam-summary"><span class="answered-dot">✓ ${answeredCount()} مجاب</span><span class="unanswered-dot">○ ${unansweredCount()} غير مجاب</span><span class="flagged-dot">🚩 ${flaggedCount()} للمراجعة</span></div></div>${q.question_code?`<div class="flh-code-inline">🔖 ${safe(q.question_code)}</div>`:''}${assetsHtml(q)}<div class="question"><b>${renderMath(q.prompt)}</b></div><div class="flh-instruction">ضغطة واحدة تختار فورًا، والحفظ يتم تلقائيًا بالخلفية.</div><div id="examV3Answers" class="answers answer-layout-v8">${opts}</div><div class="muted">${saveStatus}</div><div class="flh-exam-tools"><button class="btn btn-soft ${isFlagged?'flh-flag-on':''}" id="examFlag" ${saving||submitting?'disabled':''}>${isFlagged?'🚩 إزالة علامة المراجعة':'🚩 راجع لاحقًا'}</button></div><div class="flh-exam-nav-grid">${nav}</div><div class="flh-swipe-hint">اسحب ← للتالي، → للسابق</div><div class="flh-sticky-action flh-exam-bottom"><button class="btn btn-soft" id="examPrev" ${(index===0||saving||submitting)?'disabled':''}>السابق</button><button class="btn btn-soft" id="examNext" ${(index===questions.length-1||saving||submitting)?'disabled':''}>التالي</button><button class="btn btn-primary" id="examSubmit" ${(!allAnswered()||saving||submitting)?'disabled':''}>تسليم الامتحان</button></div><button class="btn btn-soft flh-exit-link" id="examExit" ${saving||submitting?'disabled':''}>رجوع لصفحتي — كل شيء محفوظ</button></section>`);
      ...
      zone?.addEventListener('touchend',e=>{if(!touchStart||saving||submitting)return;const t=e.changedTouches[0],dx=t.clientX-touchStart.x,dy=t.clientY-touchStart.y,target=touchStart.target;touchStart=null;if(target?.closest?.('button,.answer,input,textarea,select,a'))return;if(Math.abs(dx)<65||Math.abs(dx)<Math.abs(dy)*1.25)return;if(dx<0)navTo(index+1);else navTo(index-1);},{passive:true});
      preloadQuestion(questions[index+1]?.question);if(session.resumed)session.resumed=false;
    }

    function saveAnswer(btn){
      if(submitting)return;
      const row=questions[index],qid=row.question_id,pos=Number(btn.getAttribute('data-pos')),previous=answers.get(qid);
      answers.set(qid,pos);
    ...
    async function toggleFlag(){
      if(saving||submitting)return;const row=questions[index],next=!flagged.has(row.question_id);saving=true;render();
    ...
    async function flushAnswerSaves(){
      const pending=[...answerSaveChains.values()];
