...
- Record the TestSprite outcome as one of: `PASS`, `FINDINGS`, `BLOCKED_EXTERNAL`, or `N/A`.
- Use `BLOCKED_EXTERNAL` only when the run cannot execute because of an external provider constraint such as exhausted credits/quota, provider rate limiting, provider outage, or an external authentication/service failure. Record the concrete reason/evidence in the PR.
- `BLOCKED_EXTERNAL` is not a product-test failure and, while TestSprite remains an additive non-blocking layer, does not by itself block merge when all deterministic coverage appropriate to the changed risk is complete, the required exact-head GitHub QA gates pass, and the exact-head CodeRabbit review is clean.
- Documentation-only, metadata-only, purely internal migration mechanics, or changes that TestSprite cannot meaningfully validate may mark the run `N/A` with a concrete reason.
- Never use `BLOCKED_EXTERNAL` or `N/A` to waive missing, failing, or inappropriate deterministic coverage. If the required deterministic layer for the changed risk is absent or failing, the PR remains blocked regardless of TestSprite status.
