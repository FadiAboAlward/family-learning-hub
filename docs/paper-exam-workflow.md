...
7. Copy the approved `paper_question_count` into the quiz delivery configuration consumed by Exam queue construction: `quizzes.delivery_config.exam.question_count`. Verify that this value exactly equals the printed paper question count; never rely on the generic default (currently 10) for a paper exam.
...
Before the first production paper ingestion, the server-side start operation used by ChatGPT must be version-bound: it must accept or otherwise transactionally resolve the approved immutable `quiz_version_id`, build the queue only from that version, and persist the matching paper provenance on the attempt. If a version-bound start path is not available, do not ingest the paper through the generic latest-version path.

The queue size must also come from the approved paper question count copied into `quizzes.delivery_config.exam.question_count`; a mismatch between the printed count, version metadata, delivery config, or created queue blocks ingestion.
...
4. Allocate a unique `paper_model_code`, verify it is unused, publish the same exact questions as one versioned backend quiz/exam pool, store the immutable model-to-version binding plus `paper_content_hash`, and copy the approved question count into `quizzes.delivery_config.exam.question_count`.
5. Print and solve on paper.
6. User uploads/photos the solved pages.
7. Identify the model by `paper_model_code`, resolve it to exactly one bound `quiz_version_id`, verify the content hash and question-count handoff, and transcribe answers by printed question number using the stored question map.
...
9. Verify the created queue contains exactly the approved printed question count, then save each response using the mapped backend question id/option position.
...
- copy the same approved question count into `quizzes.delivery_config.exam.question_count` so the Exam queue cannot silently default to a different size,
...
4. Confirm the approved `paper_question_count`, `quizzes.delivery_config.exam.question_count`, and expected queue size all match the printed paper.
...
- Never ingest a paper when the approved version, question mapping, delivery-config question count, or generated queue does not exactly match the printed model.
