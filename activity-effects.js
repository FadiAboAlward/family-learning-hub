(() => {
  ...
  // Detect a real manual learner login and mark the first tracked visit as a login.
  ...
  window.fetch = async (...args) => {
    ...
    try {
      ...
      if(reqUrl.includes('/functions/v1/family-api') && typeof init.body === 'string'){
        ...
        loginRequest = b?.action === 'student_login';
      }
    } catch {}
    ...
    if(loginRequest && response.ok){
      try {
        ...
        if(d?.session) setTimeout(()=>activityApi('learner_activity',{entry_type:'login'},d.session).catch(()=>{}),0);
      } catch {}
    }
    return response;
  };
  ...
  function markActivity(){
    ...
    if(Date.now() - lastPingAt > 45000) ping('resume_after_inactivity');
  }
  async function ping(entryType='resume_after_inactivity'){
    ...
    lastPingAt = Date.now();
    try { await activityApi('learner_activity',{entry_type:entryType},token); } catch {}
  }
  ['pointerdown','keydown','touchstart'].forEach(ev=>document.addEventListener(ev,markActivity,{passive:true,capture:true}));
  document.addEventListener('visibilitychange',()=>{
    if(document.visibilityState === 'visible'){ lastInteractionAt=Date.now(); ping(); }
  });
  setInterval(()=>{
  ...
  function ensureSoundToggle(){
    ...
    if(!studentMode){ if(btn) btn.remove(); return; }
    if(!btn){
      btn=document.createElement('button'); btn.id='soundToggle'; btn.className='sound-toggle';
      btn.type='button'; btn.setAttribute('aria-label','تشغيل أو كتم أصوات النجاح');
      btn.onclick=()=>{ soundEnabled=!soundEnabled; localStorage.setItem(SOUND_KEY,String(soundEnabled)); renderSoundButton(btn); if(soundEnabled) playChime('small'); };
      document.body.appendChild(btn);
    }
    renderSoundButton(btn);
  }
  function renderSoundButton(btn){ btn.textContent=soundEnabled?'🔊':'🔇'; btn.title=soundEnabled?'كتم أصوات النجاح':'تشغيل أصوات النجاح'; }

  function playChime(level='small'){
    if(!soundEnabled) return;
    try{
      const Ctx=window.AudioContext||window.webkitAudioContext; if(!Ctx) return;
      const ctx=new Ctx(); const gain=ctx.createGain(); gain.connect(ctx.destination);
      ...
      notes.forEach((f,i)=>{ const o=ctx.createOscillator(); o.type='sine'; o.frequency.value=f; o.connect(gain); const t=ctx.currentTime+i*0.065; o.start(t); o.stop(t+0.28); });
  ...
  function celebrate(level='small'){
    ...
    const layer=document.createElement('div'); layer.className=`celebration-layer ${level}`;
    const icons=level==='big'?['🎉','✨','⭐','💫','🏆','🌟']:['✨','⭐','💫'];
    const count=level==='big'?28:12;
    for(let i=0;i<count;i++){
      const s=document.createElement('span'); s.className='celebration-piece'; s.textContent=icons[i%icons.length];
      s.style.setProperty('--x',`${Math.round(Math.random()*96)}vw`);
      s.style.setProperty('--delay',`${Math.random()*0.25}s`);
      s.style.setProperty('--drift',`${Math.round((Math.random()-.5)*120)}px`);
      s.style.setProperty('--spin',`${Math.round(Math.random()*540-270)}deg`);
      layer.appendChild(s);
    }
  ...
  function fmtMinutes(sec){
    const m=Math.round((Number(sec)||0)/60); return m<1?'أقل من دقيقة':`${m} د`;
  }
  function fmtTime(v){ return v?new Date(v).toLocaleTimeString('ar',{hour:'2-digit',minute:'2-digit'}):'—'; }
  function fmtDate(v){ return v?new Date(v).toLocaleDateString('ar',{weekday:'short',day:'numeric',month:'numeric'}):'—'; }
  async function injectParentSessionReport(){
    ...
    const h=document.querySelector('.hero h1'); if(!h || !h.textContent.includes('لوحة الأهل')) return;
    const token=parentToken(); if(!token) return;
    parentReportLoading=true;
    try{
      const d=await activityApi('parent_sessions',{},token);
      const now=Date.now(), weekStart=now-7*86400000;
      ...
      const summaries=(d.learners||[]).map(l=>{
        ...
        const visits=ss.length, logins=ss.filter(s=>s.entry_type==='login').length;
      ...
      const rows=(d.sessions||[]).slice(0,60).map(s=>{
        const l=byLearner[s.learner_id]; if(!l) return '';
        const end=s.ended_at||s.last_activity_at;
        return `<tr><td>${l.display_name}</td><td>${fmtDate(s.started_at)}</td><td>${fmtTime(s.started_at)}</td><td>${fmtTime(end)}</td><td>${fmtMinutes(s.duration_seconds)}</td><td>${s.entry_type==='login'?'دخول':'رجوع بعد توقف'}</td></tr>`;
      }).join('')||'<tr><td colspan="6">ما في جلسات مسجلة بعد.</td></tr>';
      const section=document.createElement('section'); section.className='panel'; section.id='learningSessionReport';
  ...
  const observer=new MutationObserver((mutations)=>{
    ensureSoundToggle(); filterTestFromParentUi(); injectParentSessionReport();
    for(const m of mutations){
      for(const n of m.addedNodes){
        if(!(n instanceof Element)) continue;
        const goods=[...(n.matches?.('.feedback.good')?[n]:[]),...n.querySelectorAll?.('.feedback.good')||[]];
        goods.forEach(el=>{if(el.dataset.celebrated)return;el.dataset.celebrated='1';celebrate('small');});
        const awards=[...(n.matches?.('.award-pop')?[n]:[]),...n.querySelectorAll?.('.award-pop')||[]];
        awards.forEach(el=>{if(el.dataset.celebrated)return;const t=el.textContent||'';if(/XP|أوسمة|نقطة|🏅|🎉/.test(t)){el.dataset.celebrated='1';celebrate('big');}});
      }
    }
  });
  observer.observe(document.documentElement,{childList:true,subtree:true});
  window.addEventListener('hashchange',()=>{setTimeout(()=>{ensureSoundToggle();injectParentSessionReport();},100)});
  document.addEventListener('DOMContentLoaded',()=>{ensureSoundToggle();setTimeout(injectParentSessionReport,600)});
})();